...
* 기술 트렌드 요약
   * 단기(지금~5년): “카메라 + mmWave 레이더”가 여전히 메인, LiDAR는 고급/플래그십 위주.
   * 중기: 4D 이미징 레이더 + 고해상도 카메라 + 딥러닝 기반 퓨전이 표준화.
   * 장기: 일부 OEM은 카메라 중심, 일부는 LiDAR 포함 풀-스택, 모두가 센서 퓨전 + SW 안전성(ISO 26262) + OTA 업데이트에 집중.
* 만약 직접 시스템을 설계한다면…
   * 센서 레이어: 전방 77 GHz 장거리 레이더 + 전방 카메라 + 코너 레이더 + 초음파 구성.
   * Perception: 레이더(거리/속도) + 카메라(DNN 객체 인식)를 late-fusion → 추후 deep fusion 확장.
   * 플랫폼 선택: NVIDIA/Qualcomm/Mobileye 기반 SoC 또는 자체 FPGA/SoC + 레이더/카메라 인터페이스.
   * 기능 스코프: ACC, AEB, LKA/HDA부터 시작해서 Level 2+로 확장.
